	      		...
	      		if($this->tableBody != null)
	      		{
	      			...
	      			foreach($this->tableBody->body as $rowKey=>$row)
	      			{
			            ...
			            $table .= '<td>';
			            	...
			            	if($row->priority != '')
			            	{
			            		$table .= '<button class="btn btn-sm btn-primary edit link btn-sm-c-1" data-toggle="tooltip" title="'. trans('general.reply-email').'" onclick="doCustomerCareSendEmail(\'edit\', \''.$row->$primaryKey.'\')"><i class="zmdi zmdi-email"></i></button>&nbsp;';

		            			$table .= '<button class="btn btn-sm btn-primary edit link btn-sm-c-1" data-toggle="tooltip" title="'. trans('general.view').'" onclick="doCustomerCareViewIssue(\'edit\', \''.$row->$primaryKey.'\')"><i class="zmdi zmdi-edit"></i></button>&nbsp;';
			            	}else {
			            		$table .= '<button class="btn btn-sm btn-primary edit link btn-sm-c-1" data-toggle="tooltip" title="'. trans('general.view').'" onclick="doCustomerCareViewIssue(\'edit\', \''.$row->$primaryKey.'\')"><i class="zmdi zmdi-edit"></i></button>&nbsp;';
			            	}
